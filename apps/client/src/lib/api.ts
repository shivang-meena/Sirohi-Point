...
export function registerBusiness(input: BusinessRegisterInput) {
...
export function getB2BCatalog(token: string, query: CatalogListQuery = {}) {
  return request<Product[]>(`/catalog/b2b${queryString(query)}`, { token });
}

export function getB2BProduct(token: string, id: string) {
